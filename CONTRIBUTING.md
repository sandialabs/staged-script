...
1. **Fork the Repository:**  On [`staged-script`'s main page][homepage], in
   the top-right corner, you'll see a button to [fork the repository][fork].
   Do so, and then clone your fork and set it up with
   ```bash
   git clone git@github.com:${USER}/staged-script
   cd staged-script
   git remote add upstream git@github.com:sandialabs/staged-script
   git fetch upstream
   ```
...
The [staged-script > Labels][labels] page shows you all the labels we use,
...
[labels]: https://github.com/sandialabs/staged-script/labels
...
Depending on the needs of the `staged-script` user base, a project maintainer
